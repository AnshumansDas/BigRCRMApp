...
namespace CRMApp.crmapp
{
    public partial class frmCart : System.Web.UI.Page
    {
        ...
        string StrBaseURL = ConfigurationManager.AppSettings["BaseURL"].ToString(), ServiceUrl = string.Empty, strCreatedby = string.Empty,
            strTID= string.Empty,strAPIKey = string.Empty, strPaymentURL = string.Empty;
        ...
        protected void BindCartDetails()
        {
            ...
            if (responsecart.IsSuccessStatusCode)
            {
                ...
                if (pDataTable.Rows.Count > 0)
                {
                    ...
                    lblTotalCartAmt.Text = string.Format("{0:N2}", Convert.ToDecimal(pDataTable.Rows[0]["Total_final_price"].ToString().Trim()));
                    ...
                    totalAmount = string.Format("{0:N2}", Convert.ToDecimal(pDataTable.Rows[0]["Total_final_price"].ToString().Trim()));
                }
                else
                {
        ...
        protected void btnNextPayment_ServerClick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                stateid = string.IsNullOrEmpty(LblStateId.Text.Trim()) ? 0 : Convert.ToInt16(LblStateId.Text.Trim());
                cityname = LblCity.Text.Trim();
                //countryid = Convert.ToInt16(LblCountryId.Text.Trim());
                countryid = string.IsNullOrEmpty(LblCountryId.Text.Trim()) ? 0 : Convert.ToInt16(LblCountryId.Text.Trim());
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                //web
                strTID = ConfigurationManager.AppSettings["TID"].ToString();
                strAPIKey = ConfigurationManager.AppSettings["APIKey"].ToString();
                ...
                string checksum = Hash(strAPIKey + strTID + randomnumber + cartOrderCheckoutData.orderDescription + "MYR" + totalAmount + PaymentType + "SALE" + "" + email);
                //MOBILE
        ...
        protected void lvCartList_ItemCommand(object sender, ListViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Delete")
            {
                if (commentItem != null)
                {
                    string orderid = (string)lvCartList.DataKeys[commentItem.DisplayIndex][1].ToString().Trim();
                    if (!string.IsNullOrEmpty(orderid))
                    {
                        try
                        {
                            ServiceUrl = "CRM/DeleteOrderList";
                            var crm = new crmEntity()
                            {
        ...
        protected void lvCartList_ItemDeleting(object sender, ListViewDeleteEventArgs e)
        {
        ...
        //    ListViewItem listView = (ListViewItem)ddlQty.NamingContainer;
        //    Label lblOrderId = (Label)listView.FindControl("lblOrderId");
        //    Label lblVoucherid = (Label)listView.FindControl("lblVoucherid");

        //    ServiceUrl = "CRM/UpdateOrderListQty";
        //    int userid = 0;
        //    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
        //    if (Session["username"] != null)
        //        userid = Convert.ToInt16(Session["userid"].ToString());

        //    var UpdatOL = new crmEntity()
        //    {
        //        orderid = lblOrderId.Text.Trim(),
        //        voucher_id = Convert.ToInt16(lblVoucherid.Text),
        //        product_qty = Convert.ToInt16(ddlQty.SelectedValue),
        //        user_id = userid
        //    };
        //    HttpResponseMessage response = client.PostAsJsonAsync(ServiceUrl, UpdatOL).Result;
        //    if (response.IsSuccessStatusCode)
        //    {
        //        BindCartDetails();
        //    }
        ...
        //    Label Amt = (Label)myButton.FindControl("lblTotalAmt");
        //    Label lblOrderId = (Label)myButton.FindControl("lblOrderId");
        //    ServiceUrl = "CRM/DeleteOrderList";
        //    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
        //    //int Product_ID = int.Parse(myButton.CommandArgument.ToString());
        //    var cartItem1 = new crmEntity()
        //    {
        //        orderid = lblOrderId.Text.Trim()
        //    };
        //    HttpResponseMessage response = client.PostAsJsonAsync(ServiceUrl, cartItem1).Result;
        //    if (response.IsSuccessStatusCode)
        //    {
        //        BindCartDetails();
        //        //#region CartCountItems
        //        ////======================count cart items=============
        //        //if (Session["username"] != null)
        //        //{
        //        //    ServiceUrl = "InfoApi/GetCountAddToCartByUser";
        ...
        //        //    if (responsecart.IsSuccessStatusCode)
        //        //    {
        //        //        Label sTotalCartItems = this.Master.FindControl("sTotalCartItems") as Label;
        ...
        //        //        {
        //        //            sTotalCartItems.Text = dtCart.Rows.Count.ToString();
        //        //            //ScriptManager.RegisterStartupScript(this, this.GetType(), "Pop", "ConfirmMsg();", true);
        //        //        }
        //        //        else
        //        //        { sTotalCartItems.Text = ""; }
        //        //    }
        //        //}
        //        //#endregion
        //    }
        ...
        //protected void lvCartList_ItemDataBound(object sender, ListViewItemEventArgs e)
        //{
        //    ListViewDataItem dataItem = (ListViewDataItem)e.Item;
        //    if (e.Item.ItemType == ListViewItemType.DataItem)
        //    {
        //        Label lblTotalAmt = (Label)dataItem.FindControl("lblTotalAmt");
        //        subtotal += double.Parse(lblTotalAmt.Text.Trim());
        //        ViewState["totalamount"] = subtotal.ToString("N2").Trim();
        //    }
        //}
        ...
        //            lvCartList.DataSource = pDataTable;
        //            lvCartList.DataBind();
        //            //lblTotalCartAmount.Text = string.Format("{0:N2}", Convert.ToDecimal(pDataTable.Rows[0]["TotalAmount"].ToString().Trim()));                   
        //            //lblsumAmout2.Text = string.Format("{0:N2}", Convert.ToDecimal(pDataTable.Rows[0]["TotalAmount"].ToString().Trim()));
        //            //lblsumAmout3.Text = string.Format("{0:N2}", Convert.ToDecimal(pDataTable.Rows[0]["TotalAmount"].ToString().Trim()));                    
        //            //totalAmount = string.Format("{0:N2}", Convert.ToDecimal(pDataTable.Rows[0]["TotalAmount"].ToString().Trim()));
        ...
        //        #region AbseccPayment
        //        //web
        //        string checksum = Hash("a1ac0cc290f610d5" + "METROCRM01" + randomnumber + cartOrderCheckoutData.orderDescription + "MYR" + totalAmount + rbtnPaymentMode.SelectedValue + "SALE" + "" + txtEmail.Text.Trim());
        //        //MOBILE
        //        //string checksum = Hash("93704aec1d3846b7" + "METROCRM02" + randomnumber + cartOrderCheckoutData.orderDescription + "MYR" + totalAmount + rbtnPaymentMode.SelectedValue + "SALE" + "" + txtEmail.Text.Trim());
        //        if (orderNo != string.Empty)
        //        {
        ...
        //            sb.AppendFormat("<input type='hidden' name='method' value='{0}'>", rbtnPaymentMode.SelectedValue);
        //            sb.AppendFormat("<input type='hidden' name='apiOperation' value='{0}'>", "SALE");
        //            sb.AppendFormat("<input type='hidden' name='cardType' value='{0}'>", "");
        //            sb.AppendFormat("<input type='hidden' name='checksum' value='{0}'>", checksum);
        //            // Other params go here
        ...
        //            {
        //                txtFullName.Text = DataTable.Rows[0]["user_fistname"].ToString();
        //                txtEmail.Text = DataTable.Rows[0]["email_id"].ToString();
        //                txtPhone.Text = DataTable.Rows[0]["mobile_no"].ToString();
        ...
        //    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
        //    ServiceUrl = "CRM/GetStateDetails";
        //    //var userData = new UserList()
        //    //{
        //    //    //user_id=Session["userid"]
        //    //    country_id = Convert.ToInt32(ddlcountry.SelectedValue.ToString().Trim())
        //    //};
