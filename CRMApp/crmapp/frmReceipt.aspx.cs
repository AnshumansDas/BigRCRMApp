...
namespace CRMApp.crmapp
{
    public partial class frmReceipt : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                if (respTransDetails.IsSuccessStatusCode)
                {
                    ...
                    if (dtTransactionDetails.Rows.Count > 0)
                    {
                        ...
                        lblFName.Text = dtTransactionDetails.Rows[0]["user_fistname"].ToString();
                        LblAddr1.Text = dtTransactionDetails.Rows[0]["address1"].ToString();
                        LblAddr2.Text = dtTransactionDetails.Rows[0]["address2"].ToString();
                        LblState.Text = dtTransactionDetails.Rows[0]["state"].ToString();
                        LblPinCode.Text = dtTransactionDetails.Rows[0]["postcode"].ToString();
                        LblCountry.Text = dtTransactionDetails.Rows[0]["country"].ToString();
                        LblMob.Text = dtTransactionDetails.Rows[0]["mobile_no"].ToString();
                    }
                }
            }
            catch (Exception ex)
        ...
        protected void BindCartDetails(crmEntity cart)
        {
            ServiceUrl = "CRM/GetOLPurchaseItemsByOrderNo";
            client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
            var cartData = new crmEntity()
            {
                order_no = cart.order_no
            };
            HttpResponseMessage responsePurchItemscart = client.PostAsJsonAsync(ServiceUrl, cartData).Result;
            if (responsePurchItemscart.IsSuccessStatusCode)
            {
                var PurItemsCart = responsePurchItemscart.Content.ReadAsStringAsync().Result;
                var pdtPurchItemsCart = Newtonsoft.Json.JsonConvert.DeserializeObject<System.Data.DataTable>(PurItemsCart);
                if (pdtPurchItemsCart.Rows.Count > 0)
                {
                    lvCartList.DataSource = pdtPurchItemsCart;
                    lvCartList.DataBind();
                    lblTotalCartAmt.Text = string.Format("{0:N2}", Convert.ToDecimal(pdtPurchItemsCart.Rows[0]["Total_final_price"].ToString().Trim()));
                    lblTotaSSTAmt.Text = string.Format("{0:N2}", Convert.ToDecimal(pdtPurchItemsCart.Rows[0]["TotalSST_Amount"].ToString().Trim()));
                    lblsumAmout2.Text = string.Format("{0:N2}", Convert.ToDecimal(pdtPurchItemsCart.Rows[0]["TotalAmount"].ToString().Trim()));
                    LblPromocode.Text = pdtPurchItemsCart.Rows[0]["Promocode_name"].ToString().Trim();
                }
            }
        }
        #region COMMENTED
